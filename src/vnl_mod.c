...
printf("disp %d\n",pSPARC->IP_displ[0]*ncol);
		...
		p = &pSPARC->IP_displ[0];
                for (int i = 0; i < ndc; i++) {
		//printf("fore:%d %d %d\n",n,i,atom);
                    Hx[n*DMnd + Atom_Influence_nloc[type].grid_pos[atom][i]]=0;// += Vnlx[n*ndc+i];
		    if(pSPARC->IP_displ!=p) 
			{printf("?%d\n",i);break;}
                }
	        printf("addr2.4 %p\n",pSPARC->IP_displ);
		printf("addr2.5 %d\n",pSPARC->IP_displ==NULL);
		printf("addr4 %p\n",p);
	        printf("addr2.5 %p\n",pSPARC->IP_displ);
            }
            free(Vnlx);
	    printf("addr2.5 %p\n",pSPARC->IP_displ);
	    printf("%d,wtfaaaaaaaaaaaaaaa\n",atom);
            printf("addr3 %p\n",pSPARC->IP_displ);
        }
    }
    free(alpha);
